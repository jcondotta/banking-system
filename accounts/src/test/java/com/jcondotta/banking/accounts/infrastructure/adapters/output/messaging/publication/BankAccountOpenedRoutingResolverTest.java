...
class BankAccountOpenedRoutingResolverTest {
  ...
  private final BankAccountOpenedRoutingResolver resolver = new BankAccountOpenedRoutingResolver(TOPICS_PROPERTIES);

  @Test
  void shouldCreateRouting_whenBankAccountOpenedEvent() {
    var event = bankAccountOpenedEvent();

    var routing = resolver.resolve(event);

    assertThat(resolver.domainEventType()).isEqualTo(BankAccountOpenedEvent.class);
    assertThat(routing.destination()).isEqualTo("bank-account-opened");
