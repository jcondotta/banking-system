...
import com.jcondotta.banking.accounts.infrastructure.adapters.output.messaging.properties.KafkaTopicsProperties.TopicConfig;
...
class BankAccountJointHolderAddedRoutingResolverTest {
  ...
  private static final KafkaTopicsProperties TOPICS_PROPERTIES = new KafkaTopicsProperties(
      new TopicConfig("bank-account-opened"),
      new TopicConfig("bank-account-status-changed"),
      new TopicConfig("joint-account-holder-added"),
      new TopicConfig("bank-account-activated")
  );

  private final BankAccountJointHolderAddedRoutingResolver resolver = new BankAccountJointHolderAddedRoutingResolver(TOPICS_PROPERTIES);

  @Test
  void shouldCreateRouting_whenBankAccountJointHolderAddedEvent() {
    var event = bankAccountJointHolderAddedEvent();

    var routing = resolver.resolve(event);

    assertThat(resolver.domainEventType()).isEqualTo(BankAccountJointHolderAddedEvent.class);
    assertThat(routing.destination()).isEqualTo("joint-account-holder-added");
    assertThat(routing.key()).isEqualTo(BANK_ACCOUNT_ID.asString());
  }
