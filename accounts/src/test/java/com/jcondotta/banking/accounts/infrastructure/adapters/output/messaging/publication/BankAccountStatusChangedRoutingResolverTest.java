...
class BankAccountStatusChangedRoutingResolverTest {
  ...
  private final BankAccountStatusChangedRoutingResolver resolver = new BankAccountStatusChangedRoutingResolver(TOPICS_PROPERTIES);

  @Test
  void shouldCreateRouting_whenBankAccountStatusChangedEvent() {
    var event = bankAccountStatusChangedEvent();

    var routing = resolver.resolve(event);

    assertThat(resolver.domainEventType()).isEqualTo(BankAccountStatusChangedEvent.class);
    assertThat(routing.destination()).isEqualTo("bank-account-status-changed");
