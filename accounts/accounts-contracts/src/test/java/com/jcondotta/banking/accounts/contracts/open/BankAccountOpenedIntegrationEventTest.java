...
class BankAccountOpenedIntegrationEventTest {
  ...
  private final BankAccountOpenedIntegrationPayload payload =
    new BankAccountOpenedIntegrationPayload(BANK_ACCOUNT_ID, ACCOUNT_TYPE, CURRENCY, ACCOUNT_HOLDER_ID);

  @Test
  void shouldCreateEvent_whenAllFieldsAreValid() {
    var event = new BankAccountOpenedIntegrationEvent(metadata, payload);

    assertThat(event.metadata()).isEqualTo(metadata);
    assertThat(event.payload()).isEqualTo(payload);
    assertThat(event.eventType()).isEqualTo(BankAccountOpenedIntegrationEvent.EVENT_TYPE);
  }

  @Test
  void shouldHaveExpectedEventTypeValue() {
    assertThat(BankAccountOpenedIntegrationEvent.EVENT_TYPE).isEqualTo("bank-account-opened");
  }
  ...
  void shouldSerializeEvent_whenConvertedToJson() throws Exception {
    ...
    assertThat(fieldNamesOf(payloadNode)).containsExactlyInAnyOrder("bankAccountId", "accountType", "currency", "accountHolderId");
    assertThat(payloadNode.get("bankAccountId").asText()).isEqualTo(BANK_ACCOUNT_ID.toString());
    assertThat(payloadNode.get("accountType").asText()).isEqualTo(ACCOUNT_TYPE);
    assertThat(payloadNode.get("currency").asText()).isEqualTo(CURRENCY);
  ...
  void shouldDeserializeEvent_whenJsonIsValid() throws Exception {
    ...
    var deserialized = OBJECT_MAPPER.readValue(json, BankAccountOpenedIntegrationEvent.class);
