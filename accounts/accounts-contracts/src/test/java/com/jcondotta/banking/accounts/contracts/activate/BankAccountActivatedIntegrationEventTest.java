...
class BankAccountActivatedIntegrationEventTest {
  ...
  private final BankAccountActivatedIntegrationPayload payload =
    new BankAccountActivatedIntegrationPayload(BANK_ACCOUNT_ID);
  ...
  void shouldCreateEvent_whenAllFieldsAreValid() {
    ...
    assertThat(event.eventType()).isEqualTo(BankAccountActivatedIntegrationEvent.EVENT_TYPE);
  }

  @Test
  void shouldHaveExpectedEventTypeValue() {
    assertThat(BankAccountActivatedIntegrationEvent.EVENT_TYPE).isEqualTo("bank-account-activated");
  }
  ...
  void shouldDeserializeEvent_whenJsonIsValid() throws Exception {
    ...
    var deserialized = OBJECT_MAPPER.readValue(json, BankAccountActivatedIntegrationEvent.class);
