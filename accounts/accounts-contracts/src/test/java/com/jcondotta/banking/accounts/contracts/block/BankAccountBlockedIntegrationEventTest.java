...
class BankAccountBlockedIntegrationEventTest {
  ...
  private final BankAccountBlockedIntegrationPayload payload =
    new BankAccountBlockedIntegrationPayload(BANK_ACCOUNT_ID);

  @Test
  void shouldCreateEvent_whenAllFieldsAreValid() {
    var event = new BankAccountBlockedIntegrationEvent(metadata, payload);

    assertThat(event.metadata()).isEqualTo(metadata);
    assertThat(event.payload()).isEqualTo(payload);
    assertThat(event.eventType()).isEqualTo(BankAccountBlockedIntegrationEvent.EVENT_TYPE);
  }

  @Test
  void shouldHaveExpectedEventTypeValue() {
    assertThat(BankAccountBlockedIntegrationEvent.EVENT_TYPE).isEqualTo("bank-account-blocked");
  }
  ...
  void shouldDeserializeEvent_whenJsonIsValid() throws Exception {
    ...
    var deserialized = OBJECT_MAPPER.readValue(json, BankAccountBlockedIntegrationEvent.class);
