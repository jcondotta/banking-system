...
class BankAccountClosedIntegrationEventTest {
  ...
  private final BankAccountClosedIntegrationPayload payload =
    new BankAccountClosedIntegrationPayload(BANK_ACCOUNT_ID);

  @Test
  void shouldCreateEvent_whenAllFieldsAreValid() {
    var event = new BankAccountClosedIntegrationEvent(metadata, payload);

    assertThat(event.metadata()).isEqualTo(metadata);
    assertThat(event.payload()).isEqualTo(payload);
    assertThat(event.eventType()).isEqualTo(BankAccountClosedIntegrationEvent.EVENT_TYPE);
  }

  @Test
  void shouldHaveExpectedEventTypeValue() {
    assertThat(BankAccountClosedIntegrationEvent.EVENT_TYPE).isEqualTo("bank-account-closed");
  }
  ...
  void shouldDeserializeEvent_whenJsonIsValid() throws Exception {
    ...
    var deserialized = OBJECT_MAPPER.readValue(json, BankAccountClosedIntegrationEvent.class);
