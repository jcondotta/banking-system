...
public final class LedgerAccount extends AggregateRoot<LedgerAccountId> {
    ...
    public LedgerEntry applyMovement(Movement movement, UUID transferId, Instant occurredAt) {
        var updatedBooked = balance.booked().apply(movement);
        balance = new AccountBalance(updatedBooked, balance.held());
        return LedgerEntry.of(getId(), transferId, movement.movementType(), movement.movementAmount(), occurredAt);
    }
}
