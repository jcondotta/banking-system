...
class RecipientDeletedRoutingResolverTest {
  ...
  @Test
  void shouldResolveRouting_whenRecipientDeletedEvent() {
    var event = new RecipientDeletedEvent(
      DomainEventMetadata.of(EVENT_ID, RECIPIENT_ID, OCCURRED_AT),
      new RecipientDeletedData(BANK_ACCOUNT_ID)
    );

    var routing = new RecipientDeletedRoutingResolver(TOPICS_PROPERTIES).resolve(event);

    assertThat(routing.destination()).isEqualTo("custom-recipient-deleted");
    assertThat(routing.key()).isEqualTo(BANK_ACCOUNT_ID.toString());
  }
}
