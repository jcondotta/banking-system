...
class RecipientCreatedRoutingResolverTest {
  ...
  @Test
  void shouldResolveRouting_whenRecipientCreatedEvent() {
    var event = recipientCreatedEvent();

    var routing = resolver.resolve(event);

    assertThat(resolver.domainEventType()).isEqualTo(RecipientCreatedEvent.class);
    assertThat(routing.destination()).isEqualTo("custom-recipient-created");
